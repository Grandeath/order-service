...
func newTestServer(t *testing.T) (http.Handler, *service.Service) {
	...
	return server.Router(h.Endpoints(), h.Middlewares()), svc
}
...
func TestCreate_Idempotency(t *testing.T) {
	...
	first := httptest.NewRequest(http.MethodPost, "/api/v1/orders", mustJSON(t, validCreateBody()))
	...
	second := httptest.NewRequest(http.MethodPost, "/api/v1/orders", mustJSON(t, validCreateBody()))
...
func TestList(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/orders?limit=2&offset=0", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
